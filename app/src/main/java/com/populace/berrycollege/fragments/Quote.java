...
public class Quote extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container1,
        ...
        View v = inflater.inflate(R.layout.fragment_quote, container1, false);
        if (ParseDataManager.sharedDataManager(getActivity()).CheckIsConnectedToInternet(getActivity())) {
        quote = (WebView)v.findViewById(R.id.web_quote);
        ...
        quote.setWebViewClient(new MyWebViewClient());
        bs=new BerrySession(this.getActivity());
        quote.getSettings().setJavaScriptEnabled(true);
        quote.getSettings().setPluginState(WebSettings.PluginState.ON);
        SharedPreferences settings = this.getActivity().getSharedPreferences(this.getActivity().getPackageName(), 0);
        String quotes = bs.getString(ParseDataManager.TAG_CHAR_QOUTE);
        quote.loadData(quotes, "text/html", "UTF-8");
        }
        else
        {
